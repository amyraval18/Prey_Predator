...
World::World(unsigned int seed) {
    ...
    // creates the bombs = 1
    createOrganisms(BOMB, INITIAL_BOMB); 
...
void World::display() const {
    int numAnts = 0;
    int numBugs = 0;
    int numBombs = 0; // initializing bomb bugs; to be included in display/game
    cout << endl << endl;
    for (int j = 0; j < WORLDSIZE; j++) {
        for (int i = 0; i < WORLDSIZE; i++) {
            if (grid[i][j] == NULL) {
    ...
    cout << "Ants: " << numAnts << " Bugs: " << numBugs << "  Bomb Bugs:   " << numBombs << endl;
}
    ...
    while (orgCount < count) {
        ...
        if (grid[p.x][p.y] == NULL) {
            ...
            if (orgType == ANT) {
                ...
                new Bomb(this, p.x, p.y); // Creates a Bomb bug and puts it into the world 
            }
        }
    }
}
